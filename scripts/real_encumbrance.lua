...
function computePenalties(nodePC)
	...
	if table.getn(eqcheckpenaltytable) ~= 0 then
		table.insert(checkpenaltytable, TotalEncumbranceLib.tableSum(eqcheckpenaltytable)) -- add equipment total to overall table for comparison with encumbrance
	end
	...
	if table.getn(spellfailuretable) ~= 0 then
		spellfailuretoset = TotalEncumbranceLib.tableSum(spellfailuretable) -- this would sum penalties on multi-equipped armor
	else
		spellfailuretoset = 0
	end
